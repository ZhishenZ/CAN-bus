...
int mcc118_continuous_scan(void)
{
    ...
    do
    {
        ...
        usleep(200000);/* read the sensor data from MCC board every 200 ms*/
        ...
        /* if the data recording switch is ON, record data */
        if (data_recording_active)
        {
            /*if the log file is not created*/
            if (!log_file_ptr)
            {
                if (!create_mcc118_log_file(&log_file_ptr, channel_mask,
            ...
            /* If the logging time is reached, close the current file and start a new logging file. */
...
#ifdef PRINT_DATA_IN_TERMINAL /* only print is the PRINT_DATA_IN_TERMINAL macro is defined */
        ...
        else if (log_file_ptr) /* data recording switch is OFF but the log_file_ptr is not NULL*/
        {
